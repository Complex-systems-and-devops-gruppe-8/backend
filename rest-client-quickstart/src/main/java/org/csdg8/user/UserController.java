...
public class UserController {
    ...
    public Entity getUsers() throws Siren4JException {
        ...
        List<GetUserResponse> userResponses = users.stream()
                .map(user -> new GetUserResponse(user.id, user.username, user.role.toString(), user.linkedGames))
    ...
    public Entity getUser(Long id) throws Siren4JException {
        ...
        GetUserResponse userResponse = new GetUserResponse(user.id, user.username, user.role.toString(), user.linkedGames);
